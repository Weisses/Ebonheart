...
public class EABlockTorchSoularite extends BlockTorch {
	    ...
	    public void randomDisplayTick(World pos, BlockPos state, IBlockState worldIn, Random rand)
	    {
	        ...
	        if (enumfacing.getAxis().isHorizontal())
	        {
	            ...
	            pos.spawnParticle(EnumParticleTypes.REDSTONE, d0 + d4 * (double)enumfacing1.getFrontOffsetX(), d1 + d3, d2 + d4 * (double)enumfacing1.getFrontOffsetZ(), 1.0D, 5.0D, 0.0D, new int[0]);
	        }
	        else
	        {
	            pos.spawnParticle(EnumParticleTypes.SPELL_MOB_AMBIENT, d0, d1, d2, 1.0D, 5.0D, 0.0D, new int[0]);
	            pos.spawnParticle(EnumParticleTypes.REDSTONE, d0, d1, d2, 1.0D, 5.0D, 0.0D, new int[0]);
	        }
	    }
