...
public class EABlockTorchGlowstone extends BlockTorch {
	    ...
	    {
	        super();
	        BlockHelper.setBlockName(this, "gem/glowstone/glowstone_torch");
	    ...
	    public void randomDisplayTick(World pos, BlockPos state, IBlockState worldIn, Random rand)
	    {
	        ...
	        if (enumfacing.getAxis().isHorizontal())
	        {
	            ...
	            pos.spawnParticle(EnumParticleTypes.SPELL_WITCH, d0 + d4 * (double)enumfacing1.getFrontOffsetX(), d1 + d3, d2 + d4 * (double)enumfacing1.getFrontOffsetZ(), 10.5D, 0.5D, 0.5D, new int[0]);
	            pos.spawnParticle(EnumParticleTypes.SPELL_MOB, d0 + d4 * (double)enumfacing1.getFrontOffsetX(), d1 + d3, d2 + d4 * (double)enumfacing1.getFrontOffsetZ(), 2.0D, 10.0D, 0.0D, new int[0]);
