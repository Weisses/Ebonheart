...
public class BlockDragonwoodPlantRoot extends Block implements ITileEntityProvider
{
	...
	public BlockDragonwoodPlantRoot(Item Fruit, int FruitFallHeight, int FruitMinAmount, int FruitMaxAmount, float PlantHardness, float PlantResistance, int MinimumGrowLightLevel, int GrowChancePercentage, int UpdateTickInterval, int MinGenHeight, int MaxGenHeight, int MaxGenAmountPerChunk, int GenChancePercentage, int SpawnChildChancePercentage, int SpawnChildMaxDistance, BiomeGenBase[] GenBiomes)
	{
		...
		this.setUnlocalizedName("dragonwoodplant");
    	...
    	this.dragonwoodplantsection = new BlockDragonwoodPlantSection(this, Fruit, FruitFallHeight, FruitMinAmount, FruitMaxAmount, PlantHardness, PlantResistance, BlockDragonwoodPlantRoot.HarvestToolEnum.AXE, BlockDragonwoodPlantRoot.HarvestLevelEnum.WOOD);
    	
    	
    	//GameRegistry.registerBlock(this, "dragonwoodplant");
    	//ClientRegistry.bindTileEntitySpecialRenderer(TileEntityDragonwoodPlant.class, new TileEntityDragonwoodPlantRenderer());
    	//GameRegistry.registerTileEntity(TileEntityDragonwoodPlant.class, "TileEntityDragonwoodPlant");
	}
    ...
    {
    	return (block == Blocks.air || block == this.dragonwoodplantsection);
    }
	...
	public void KillPlant(World world, BlockPos pos, boolean dropitems)
    {
		...
		for(int i = 0; i <= 3; i++)
		{
			if(world.getBlockState(pos.up(i)).getBlock() == this || world.getBlockState(pos.up(i)).getBlock() == this.dragonwoodplantsection)
			{				
	...
	{				
		...
		if(this.hasRoomForAge(world, pos, age))
		{
			for (int i = 1; i < 4; i++)
			{
				if(world.getBlockState(pos.up(i)).getBlock() == this.dragonwoodplantsection)
				{
					world.setBlockToAir(pos.up(i));
				}				
			}
			...
			else if(age == 1)
	    	{
	    		...
	    		world.setBlockState(pos.up(1), this.dragonwoodplantsection.getDefaultState());
	    	...
	    	else if(age >= 2)
	    	{
				...
				world.setBlockState(pos.up(2), this.dragonwoodplantsection.getDefaultState());
	        	childtileentity = world.getTileEntity(pos.up(2));
				if(childtileentity instanceof TileEntityDragonwoodPlant)
				{
					((TileEntityDragonwoodPlant) childtileentity).setAgeTier(age,3);
				}
				world.setBlockState(pos.up(3), this.dragonwoodplantsection.getDefaultState());
	...
	public void KillIfInvalid(World world, BlockPos pos)
    {
		...
		if(DragonwoodPlant != null)
		{		
	    	if(DragonwoodPlant.getAge() == 1)
	    	{
	    		if(world.getBlockState(pos.up(1)).getBlock() != this.dragonwoodplantsection)
	    		{
	    			this.KillPlant(world, pos, false);
	    		}
	    	}
	    	else if(DragonwoodPlant.getAge() >= 2)
	    	{    		
	    		if(world.getBlockState(pos.up(1)).getBlock() != this.dragonwoodplantsection || world.getBlockState(pos.up(2)).getBlock() != this.dragonwoodplantsection || world.getBlockState(pos.up(3)).getBlock() != this.dragonwoodplantsection)
	    		{
	    			this.KillPlant(world, pos, false);
	    		}
	    	}
			...
			if(DragonwoodPlant != null)
			{
				if(DragonwoodPlant.getAge() < 7)
				{
					...
					{		
						...
						{				
							this.setPlantAge(world, pos, DragonwoodPlant.getAge()+1);
						}
					}		
				}
				else
