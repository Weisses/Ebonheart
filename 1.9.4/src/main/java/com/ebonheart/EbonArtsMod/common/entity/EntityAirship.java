...
public class EntityAirship extends Entity {
    ...
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirship.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirship.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirship.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean>[] DATA_ID_PADDLE = new DataParameter[] {EntityDataManager.createKey(EntityAirship.class, DataSerializers.BOOLEAN), EntityDataManager.createKey(EntityAirship.class, DataSerializers.BOOLEAN)};
    ...
    private EntityAirship.Status status;
    private EntityAirship.Status previousStatus;
    ...
    public EntityAirship(World worldIn, double x, double y, double z)
    {
        this(worldIn);
    ...
    protected void entityInit()
    {
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirship.Type.OAK.ordinal()));
    ...
    {
        if (entityIn instanceof EntityAirship)
        {
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirship.Status.UNDER_WATER && this.status != EntityAirship.Status.UNDER_FLOWING_WATER)
        {
    ...
    private EntityAirship.Status getBoatStatus()
    {
        ...
        {
            return EntityAirship.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirship.Status.ON_LAND;
            }
            else
            {
                return EntityAirship.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirship.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
        ...
        return flag ? EntityAirship.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirship.Status.IN_AIR && this.status != EntityAirship.Status.IN_AIR && this.status != EntityAirship.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirship.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirship.Status.IN_WATER)
            {
                d2 = (this.waterLevel - this.getEntityBoundingBox().minY) / (double)this.height;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityAirship.Status.UNDER_FLOWING_WATER)
            {
                d1 = -7.0E-4D;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityAirship.Status.UNDER_WATER)
            {
                d2 = 0.009999999776482582D;
                this.momentum = 0.45F;
            }
            else if (this.status == EntityAirship.Status.IN_AIR)
            {
                this.momentum = 0.9F;
            }
            else if (this.status == EntityAirship.Status.ON_LAND)
            {
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirship.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos)
    {
        ...
        if (!this.isRiding())
        {
            if (onGroundIn)
            {
                ...
                {
                    if (this.status != EntityAirship.Status.ON_LAND)
                    {
    ...
    public EntityAirship.Type getBoatType()
    {
        return EntityAirship.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirship.Type byId(int id)
        {
        ...
        public static EntityAirship.Type getTypeFromString(String nameIn)
        {
