...
public class EABlockTorchNecrocite extends Block {
	    ...
	    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
	    {
	        ...
	        if (enumfacing.getAxis().isHorizontal())
	        {
	            ...
	            worldIn.spawnParticle(EnumParticleTypes.REDSTONE, d0 + d4 * (double)enumfacing1.getFrontOffsetX(), d1 + d3, d2 + d4 * (double)enumfacing1.getFrontOffsetZ(), -9.0D, 9.0D, -9.0D, new int[0]);
	        }
	        else
	        {
	            worldIn.spawnParticle(EnumParticleTypes.SPELL_MOB_AMBIENT, d0, d1, d2, 0.0D, 5.0D, 0.0D, new int[0]);
	            worldIn.spawnParticle(EnumParticleTypes.REDSTONE, d0, d1, d2, -9.0D, 9.0D, -9.0D, new int[0]);
	        }
	    }
	    ...
	    public int getMetaFromState(IBlockState state)
	    {
	        ...
	        switch (EABlockTorchNecrocite.SwitchEnumFacing.FACING_LOOKUP[((EnumFacing)state.getValue(FACING)).ordinal()])
	        {
	            case 1:
	                i = b0 | 1;
	                break;
