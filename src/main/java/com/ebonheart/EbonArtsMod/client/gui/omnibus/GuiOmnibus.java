...
import com.ebonheart.EbonArtsMod.api.omnibus.BotaniaTutorialStartEvent;
import com.ebonheart.EbonArtsMod.api.omnibus.OmnibusCategory;
import com.ebonheart.EbonArtsMod.api.omnibus.OmnibusEntry;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonAchievement;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonBookmark;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonCategory;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonChallengeInfo;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonChallenges;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonDoot;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonHistory;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonInvisible;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonNotes;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonOptions;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonUpdateWarning;
import com.ebonheart.EbonArtsMod.common.omnibus.LexiconData;
import com.ebonheart.EbonArtsMod.common.omnibus.page.PageText;
import com.ebonheart.EbonArtsMod.proxy.ClientProxy;
...
public class GuiOmnibus extends GuiScreen {

	public static GuiOmnibus currentOpenLexicon = new GuiOmnibus();
	...
	public static List<GuiOmnibus> bookmarks = new ArrayList();
	public static List<String> bookmarkKeys = new ArrayList();
	boolean bookmarksNeedPopulation = false;

	public static Queue<OmnibusEntry> tutorial = new ArrayDeque();
	...
	List<OmnibusCategory> allCategories;
	...
	public void onInitGui() {
		allCategories = new ArrayList(EbonArtsAPI.getAllCategories());
		...
		if(isIndex()) {
			...
			for(int i = 0; i < 12; i++) {
				int y = 16 + i * 12;
				buttonList.add(new GuiButtonInvisible((GuiOmnibusIndex) this, i, left + x, top + y, 110, 10, ""));
			}
			populateIndex();
		} else if(isCategoryIndex()) {
			int categories = allCategories.size();
			for(int i = 0; i < categories + 1; i++) {
				OmnibusCategory category = null;
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		if(par1GuiButton.id >= BOOKMARK_START) {
			if(par1GuiButton.id >= BOOKMARK_START + MAX_BOOKMARK_COUNT) {
				if(par1GuiButton instanceof GuiButtonChallengeInfo)
					mc.displayGuiScreen(new GuiOmnibusEntry(LexiconData.challenges, this));
				else mc.displayGuiScreen(new GuiOmnibusHistory());
				ClientTickHandler.notifyPageChange();
			} else handleBookmark(par1GuiButton);
		} else if(par1GuiButton instanceof GuiButtonCategory) {
			OmnibusCategory category = ((GuiButtonCategory) par1GuiButton).getCategory();

			mc.displayGuiScreen(new GuiOmnibusIndex(category));
			ClientTickHandler.notifyPageChange();
		...
		case -2 :
			...
			break;
		case -3 :
			mc.displayGuiScreen(new GuiOmnibusChallengesList());
			break;
		case -4 :
			if(isShiftKeyDown()) {
				try {
	...
	public void handleBookmark(GuiButton par1GuiButton) {
		...
		if(i == bookmarks.size()) {
			...
			if(isShiftKeyDown()) {
				...
				GuiOmnibus bookmark = bookmarks.get(i).copy();
	...
	void populateIndex() {
		List<OmnibusCategory> categoryList = EbonArtsAPI.getAllCategories();
		int shift = 2;
		for(int i = shift; i < 12; i++) {
			int i_ = i - shift;
			GuiButtonInvisible button = (GuiButtonInvisible) buttonList.get(i);
			OmnibusCategory category = i_ >= categoryList.size() ? null : categoryList.get(i_);
			if(category != null)
	...
	void populateBookmarks() {
		...
		for(int i = 0; i < len + (addEnabled ? 1 : 0); i++) {
			...
			GuiOmnibus gui = isAdd ? null : bookmarks.get(i);
	...
	public void positionTutorialArrow() {
		OmnibusEntry entry = tutorial.peek();
		OmnibusCategory category = entry.category;
	...
	public static GuiOmnibus create(NBTTagCompound cmp) {
		String type = cmp.getString(TAG_TYPE);
		try {
			GuiOmnibus lex = (GuiOmnibus) Class.forName(type).newInstance();
			if(lex != null)
				lex.load(cmp);
	...
	public GuiOmnibus copy() {
		return new GuiOmnibus();
	}

	public static boolean isValidLexiconGui(GuiOmnibus gui)	{
		if(gui == null)
			return false;
		if(gui.isCategoryIndex() || gui.isChallenge())
			return true;
		if(gui.isIndex()) {
			GuiOmnibusIndex indexGui = (GuiOmnibusIndex) gui;
			if(indexGui.category == null)
				return true;
			return EbonArtsAPI.getAllCategories().contains(indexGui.category);
		}

		GuiOmnibusEntry entryGui = (GuiOmnibusEntry) gui;
		if(!EbonArtsAPI.getAllEntries().contains(entryGui.entry))
			return false;
