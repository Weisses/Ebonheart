...
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonBack;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonInvisible;
import com.ebonheart.EbonArtsMod.client.gui.omnibus.button.GuiButtonPage;
...
public class GuiOmnibusIndex extends GuiOmnibus implements IParented {

	private static final String TAG_CATEGORY = "category";
	private static final String TAG_PAGE = "page";

	OmnibusCategory category;
	String title;
	int page = 0;
	...
	OmnibusEntry currentEntry;
	float infoTime;

	List<OmnibusEntry> entriesToDisplay = new ArrayList();

	public GuiOmnibusIndex() {
		parent = new GuiOmnibus();
	}

	public GuiOmnibusIndex(OmnibusCategory category) {
	...
	public void onInitGui() {
		...
		if(!GuiOmnibus.isValidLexiconGui(this))	{
			currentOpenLexicon = new GuiOmnibus();
	...
	void buildEntries() {
		...
		for(OmnibusEntry entry : category == null ? EbonArtsAPI.getAllEntries() : category.entries) {
	...
	@Override
	void populateIndex() {
		OmnibusEntry tutEntry = tutorial != null && !tutorial.isEmpty() ? tutorial.peek() : null;

		for(int i = page * 12; i < (page + 1) * 12; i++) {
			GuiButtonInvisible button = (GuiButtonInvisible) buttonList.get(i - page * 12);
			OmnibusEntry entry = i >= entriesToDisplay.size() ? null : entriesToDisplay.get(i);
			if(entry != null) {
	...
	void openEntry(int index) {
		...
		OmnibusEntry entry = entriesToDisplay.get(index);
		mc.displayGuiScreen(new GuiOmnibusEntry(entry, this));
		ClientTickHandler.notifyPageChange();
	}
	...
	@Override
	public void setParent(GuiOmnibus gui) {
		parent = gui;
	}
	...
	protected void keyTyped(char par1, int par2) {
		if(par2 == 203 || par2 == 200 || par2 == 201) // Left, Up, Page Up
			...
			mc.displayGuiScreen(new GuiOmnibus());
			ClientTickHandler.notifyPageChange();
	...
	public void load(NBTTagCompound cmp) {
		...
		if(categoryStr.isEmpty())
			category = null;
		else for(OmnibusCategory cat : EbonArtsAPI.getAllCategories())
	...
	public GuiOmnibus copy() {
		GuiOmnibusIndex gui = new GuiOmnibusIndex(category);
		gui.page = page;
		gui.setTitle();
		return gui;
	}
