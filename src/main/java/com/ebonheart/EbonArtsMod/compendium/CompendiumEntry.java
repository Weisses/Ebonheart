...
public abstract class CompendiumEntry implements Comparable<CompendiumEntry>{
	...
	public final String getDescription(String subItemID){
		for (CompendiumEntry entry : subItems){
			if (entry.getID().equals(subItemID)){
				return description + " " + AlchemicalCompendium.KEYWORD_NEWLINE + " " + AlchemicalCompendium.KEYWORD_NEWLINE + " " + entry.getDescription();
			}
		}
	...
	public void parse(Node node){
		...
		for (int i = 0; i < childNodes.getLength(); ++i){
			...
			if (childNode.getNodeName() == "name"){
				...
				AlchemicalCompendium.instance.addAlias(subItem.getID(), this.getID());
			}
		}
	...
	public CompendiumEntry[] getRelatedItems(){
		...
		for (String s : this.relatedItems){
			CompendiumEntry e = AlchemicalCompendium.instance.getEntry(s);
			if (e != null && e != this)
				relations.add(e);
		}
	...
	public GuiAlchemicalCompendium getCompendiumGui(String searchID){
		int meta = -1;
	...
	protected
	abstract GuiAlchemicalCompendium getCompendiumGui(String searchID, int meta);
