 ...
 */
package com.ebonheart.EbonArtsMod.common.omnibus.page;
...
public class PageRecipe extends OmnibusPage {
	...
	public void renderScreen(IGuiOmnibusEntry gui, int mx, int my) {
		relativeMouseX = mx;
		relativeMouseY = my;
	...
	@SideOnly(Side.CLIENT)
	public void renderRecipe(IGuiOmnibusEntry gui, int mx, int my) {
		// NO-OP
	}

	@SideOnly(Side.CLIENT)
	public void renderItemAtAngle(IGuiOmnibusEntry gui, float angle, ItemStack stack) {
	...
	@SideOnly(Side.CLIENT)
	public void renderItemAtGridPos(IGuiOmnibusEntry gui, int x, int y, ItemStack stack, boolean accountForContainer) {
		if(stack == null || stack.getItem() == null)
			return;
		stack = stack.copy();
	...
	@SideOnly(Side.CLIENT)
	public void renderItem(IGuiOmnibusEntry gui, double xPos, double yPos, ItemStack stack, boolean accountForContainer) {
		...
		if(relativeMouseX >= xpi && relativeMouseY >= ypi && relativeMouseX <= xpi + 16 && relativeMouseY <= ypi + 16) {
			...
			EntryData data = OmnibusRecipeMappings.getDataForStack(tooltipStack);
			ItemStack book = Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem();

			if(data != null && (data.entry != gui.getEntry() || data.page != gui.getPageOn()) && book != null && book.getItem() instanceof IOmnibus && ((IOmnibus) book.getItem()).isKnowledgeUnlocked(book, data.entry.getKnowledgeType())) {
				tooltipEntry = true;

				if(!mouseDownLastTick && mouseDown && GuiScreen.isShiftKeyDown()) {
					GuiOmnibusEntry newGui = new GuiOmnibusEntry(data.entry, (GuiScreen) gui);
